...
namespace ds::amt {
    ...
    {
        BlockType* rmBLock = accessNext(block);
        if (rmBLock == accessLast()) {
            this->removeLast();
        }
        else {
            this->disconnectBlock(rmBLock);
            AMS<BlockType>::memoryManager_->releaseMemory(rmBLock);
        }
    }
    ...
    {
        BlockType* rmBLock = accessPrevious(block);
        if (rmBLock == first_) {
            this->removeFirst();
        }
        else {
