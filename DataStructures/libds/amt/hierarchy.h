...
namespace ds::amt {
	...
	{
		const BlockType *cur = &node;
		size_t count = 0;
		while (cur != nullptr) {
			cur = this->accessParent(*cur);
			++count;
		}
		return count - 1;
	}
	...
	{
		if (node != nullptr) {
			BlockType* tmpSon = this->accessLeftSon(*node);
			if (tmpSon != nullptr) {
				this->processPreOrder(tmpSon, operation);
			}
			operation(node);
			tmpSon = this->accessRightSon(*node);
			if (tmpSon != nullptr) {
				this->processPreOrder(tmpSon, operation);
			}
		}
